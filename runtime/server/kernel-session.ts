...
import type { RestrictedEvidenceRecorder } from '../../src/core/agent-tools/restricted-evidence.js';
...
export interface KernelChatResponse {
  ...
  readonly findingMetadata?: readonly {
    ...
    readonly channel: 'output' | 'error' | 'stderr' | 'partial' | 'metadata';
...
export interface KernelChatSessionOptions {
  /**
   * OPERATIONAL ADMISSION is not an option on this interface, deliberately.
   *
   * A caller-supplied purpose and authority used to sit here, and an audit rejected the pair as
   * a bypass: a claim the caller makes about itself is not an authorization. `admitWork` now
   * admits on the committed governed status alone, `runAgent` passes a literal category, and
   * nothing a session is constructed with is consulted. See ADR-006.
   */
  ...
  /** Restricted forensic sink; the common runtime guard itself is mandatory. */
  ...
  /** Stable room identity used as one component of background-process ownership. */
...
/** Per-turn authority that further narrows this session's private process registry. */
...
export class KernelChatSession {
  ...
  /** Private capability container: never returned through the public session/server API. */
  private readonly processRegistry = createProcessRegistry();
  private processSessionId = `kernel-${randomUUID()}`;
  private disposed = false;

  constructor(opts: KernelChatSessionOptions) {
    if (!Array.isArray(opts.toolNames)) throw new Error('KernelChatSession requires an explicit tool lane');
    if (new Set(opts.toolNames).size !== opts.toolNames.length) throw new Error('KernelChatSession tool lane contains duplicates');
    this.opts = Object.freeze({ ...opts, toolNames: Object.freeze([...opts.toolNames]) });
  ...
  getToolNames(): readonly string[] { return [...this.opts.toolNames]; }

  getTokenSummary(): ReturnType<TokenMonitor['summary']> { return this.tokenMonitor.summary(); }
  ...
  reset(): void {
    // A reset expires every process capability from the old logical session.
    this.processRegistry.clearSession(this.processSessionId);
    this.processSessionId = `kernel-${randomUUID()}`;
    this.history = [];
    this.turnCount = 0;
  ...
  /** Session eviction/runtime shutdown cleanup. Unrelated session registries are unreachable. */
  dispose(): void {
  ...
  ): Promise<KernelChatResponse> {
    return this.send(userMessage, onEvent, extraContext, processIdentity);
  }
  ...
  async send(
    ...
    if (this.disposed) throw new Error('kernel session is disposed');
    ...
    const result = await runAgent({
      ...
      ...(this.opts.evidenceRecorder !== undefined ? { evidenceRecorder: this.opts.evidenceRecorder } : {}),
      ...(this.opts.clock !== undefined ? { clock: this.opts.clock } : {}),
      processScope: this.processRegistry.scope({
        sessionId: this.processSessionId,
        roomKey: this.opts.roomKey ?? 'direct',
        taskId: processIdentity?.taskId ?? this.opts.taskId ?? 'kernel-session',
        callerId: processIdentity?.callerId ?? 'direct',
      }),
    });
    ...
    return {
      ...
      findingMetadata: events
        .filter((event): event is Extract<AgentEvent, { kind: 'velum-finding' }> => event.kind === 'velum-finding')
        .map((event) => ({
