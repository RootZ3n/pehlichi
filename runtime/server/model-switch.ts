 ...
 * resolves for a target. AGENT_MODEL_TARGETS (JSON) can add more; a custom POST /model can
 ...
 * (AGENT_BASE_URL for Mimo, DEEPSEEK_BASE_URL for DeepSeek); AGENT_MODEL_TARGETS adds more.
 */
export function availableModelTargets(env: NodeJS.ProcessEnv = process.env): ModelTarget[] {
  ...
  for (const t of parseTargetsEnv(env.AGENT_MODEL_TARGETS)) {
