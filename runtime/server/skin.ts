...
export function loadSkin(configuredPath?: string): Skin {
  const skinPath = configuredPath ?? join(__dirname, '..', '..', 'skin.yaml');
  const cachedSkin = cache.get(skinPath);
