...
const root=path.resolve(path.dirname(fileURLToPath(import.meta.url)),'../..');const read=(p)=>parseStrictJsonText(fs.readFileSync(path.join(root,p),'utf8'));
test('TRIO-001D closed boundary carries labeled slots, a closed inventory, and no implied executableAllowed field',()=>{const m=read('trio/governance/boundary-manifest.json');assert.equal(m.operationalStatus.state,'PRE_PRODUCTION');assert.equal(m.operationalStatus.authorization,'NOT_AUTHORIZED_FOR_OPERATIONAL_WORK');assert.equal(m.operationalStatus.productionAgent,'hermes');assert.equal(m.schemaVersion,'2.0.0');assert.deepEqual(Object.keys(m.repositories).sort(),['loony-luna','mad-ptah','pehlichi']);assert.equal(JSON.stringify(m).includes('executableAllowed'),false);assert.equal(m.pathPolicy.unknownFiles,'fail-closed');assert.equal(m.pathInventory.path,'path-inventory.json');assert.equal(m.pathInventory.schemaVersion,'1.0.0');assert.ok(m.rules.filter((x)=>x.selector.directory).every((x)=>x.selector.closedInventory===true));});
