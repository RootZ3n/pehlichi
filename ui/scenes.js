...
// data. Renderers are async, return HTML, and degrade to a friendly note
// when the backend is unreachable. defIds MUST match WORKSPACE_REGISTRY in
// index.html; scene mapping follows each area's stated purpose.
// ══════════════════════════════════════════════════════════════════════
(function () {
  'use strict';
  ...
  function offline(r) {
    return '<div class="peh-live-off"><b>The line to the settlement is quiet.</b><span>' +
      esc(r && r.error ? r.error : 'backend unreachable') +
      '</span><span class="peh-live-off-hint">Is Peh\'s server up on :18830? *tail twitch*</span></div>';
  }
  ...
  async function rSessions() {
    ...
    var d = r.data;
    var head = '<div class="peh-stats">' + stat('active', d.count) + stat('converse', d.converseSessions != null ? d.converseSessions : 0) + stat('evicted', d.evicted) + '</div>';
  ...
  async function rAgents(skills) {
    var r = await window.PehAPI.agents();
    if (!r.ok || !r.data) return offline(r);
    var d = r.data;
    var self = d.self ? '<div class="peh-stats">' + stat('my tools', d.self.tools) + stat('peers', d.count) + '</div>' : '';
    var list = (d.agents || []).map(function (a) {
      return row(a.name, 'port ' + a.port + ' · ' + a.status, dot(a.status === 'available') + '<span class="peh-row-desc">' + esc(a.description) + '</span>');
    });
  ...
  async function rTools() {
    ...
    var head = '<div class="peh-stats">' + stat('tools', t.data.count) +
      (h.ok && h.data ? stat('uptime', ago((h.data.uptime || 0) * 1000)) + stat('commit', h.data.commit || '—') : stat('health', 'down')) + '</div>';
    var names = (t.data.tools || []).map(function (n) { return '<span class="peh-chip">' + esc(n) + '</span>'; }).join('');
  ...
  async function rMissions() {
    ...
    var ql = queue.length ? rows(queue.map(function (q) { return row(q.roomKey, 'active · ' + q.historyLength + ' msgs'); })) : '<p class="peh-live-empty">No quests in progress. The board is clear.</p>';
  ...
  async function rMemories(full) {
    ...
    if (!r.ok || !r.data) return offline(r);
    var d = r.data;
    var head = '<div class="peh-stats">' + stat('past lives', (d.pastLives || []).length) + stat('curated', (d.entries || []).length) + '</div>';
    var lives = (d.pastLives || []).map(function (l) {
      var open = !!expanded[l.project];
      var detail = open ? '<div class="peh-mem-detail"><p>' + esc(l.traits) + '</p>' +
        (l.speechQuirks ? '<p class="peh-mem-quirk">“' + esc(l.speechQuirks) + '”</p>' : '') + '</div>' : '';
      return '<button class="peh-mem ' + (open ? 'open' : '') + '" type="button" onclick="PehScenes.toggleLife(\'' + esc(l.project) + '\')">' +
        '<span class="peh-mem-h"><span class="peh-mem-name">' + esc(l.name) + '</span><span class="peh-mem-era">' + esc(l.era) + '</span></span>' +
        detail + '</button>';
    }).join('');
  ...
  async function rIdentity() {
    ...
    var d = i.data || {};
    var ad = a.data || {};
    var head = '<div class="peh-stats">' + stat('model', ad.model || d || '—') + stat('status', ad.status || 'active') + '</div>';
  ...
  async function rKnowledge() {
    ...
    var el = entries.length ? rows(entries.map(function (e) { return row(e.title, e.project + ' · ' + e.status, '<span class="peh-row-desc">' + esc(e.description || '') + '</span>'); }))
      : '<p class="peh-live-empty">The archive shelves are bare — no curated entries yet.</p>';
    return head + el;
  }
  // The hub overview console summary strip (health + reach).
  ...
  // ── defId → renderer + actionable options (Peh's "what's available") ──────
  var MAP = {
    'console': { fn: rConsole },
  ...
  function liveContainer(defId) {
    return '<div class="peh-live" id="peh-live-' + esc(defId) + '">' +
      optionRow(defId) +
      '<div class="peh-live-body"><div class="peh-live-loading">Peh is fetching the latest… <span class="peh-live-spin"></span></div></div></div>';
  }
