...
} from './lib/truth-agent-adapter.js';
import { TruthSessionGate, type ObservedToolCall } from './lib/truth-gate.js';
...
/** Which package name belongs to which agent. Identity travels with the tree, not the path. */
const PACKAGE_IDENTITY: Readonly<Record<string, string>> = {
  'peh-tui': 'pehlichi',
  'luna-tui': 'loony-luna',
  'ptah-tui': 'mad-ptah'
};
...
function selectIdentity(argv: readonly string[]): string {
  ...
  try {
    packageName = String((JSON.parse(readFileSync(join(TUI_ROOT, 'package.json'), 'utf8')) as { name?: unknown }).name ?? '');
  } catch (error) {
    refuse('profile-unknown', `the harness could not read the package it is running inside: ${(error as Error).message}`);
  }
  const owner = PACKAGE_IDENTITY[packageName];
  if (!owner) refuse('profile-unknown', `package "${packageName}" is not one of the three Trio runtimes`);
  if (owner !== value) {
